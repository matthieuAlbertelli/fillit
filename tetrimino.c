...
/*   Updated: 2018/04/20 23:21:46 by malberte         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char		*read_line(unsigned int pos[NB_BLOCKS][2], int *nb_blocks, const char *buf, int height)
{
	int w;

	w = 0;
	while (w < NB_BLOCKS && buf[w] && *nb_blocks < NB_BLOCKS)
	{
		if (buf[w] == '#')
		{
...
static char		*read_tetrimino(t_tetrimino *new, int *nb_tetri, const char *buf)
{
	...
	//Chercher le tetrimino dans le dictionnaire
	pat = ft_pattern_recognition(*g_patterns, pos);
	if (pat == NULL)
	{
